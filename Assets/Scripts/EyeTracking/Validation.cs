...
public class Validation : MonoBehaviour
{
    ...
    private void ValidationAnimation()
    {
        ...
        transform.position =  player.transform.position + player.transform.rotation * Vector3.Lerp(keyPositions[_validationPointIdx], keyPositions[_validationPointIdx-1], _animationTime / 1f); //TODO: check if the animation works smoothly with new time update
    ...
    void FixedUpdate()
    {
        ...
        if ((_validationPointIdx < keyPositions.Count - 1) && doingValidation)
        {
            ...
            if (_fixationTime > 0 && fixationOngoing)
            {
                ...
                transform.position = player.transform.position + player.transform.rotation * keyPositions[_validationPointIdx];
                // LookAt to ensure the points are always in front of the camera
                transform.LookAt(player.transform);
                ...
                var hmdTransform = player.transform;
