		...
		//	ArrayList prefabArray = new ArrayList(); prefabArray.Clear();		// <- i dont think this is used [DELETE LATER]
			foreach( MeshFilter mf in allMeshfilters ){
		        ...
		        if( mf.sharedMesh != null && // Make sure there IS a shared Mesh in the first place!
		        	PrefabUtility.GetPrefabObject(mf.gameObject) != null && // This lets us know if this object is actually a prefab
		        	PrefabUtility.FindRootGameObjectWithSameParentPrefab(mf.gameObject) != null &&	// Can we grab the parent prefab
		...
		//	ArrayList prefabArray = new ArrayList(); prefabArray.Clear();		// <- i dont think this is used [DELETE LATER]
			foreach( SkinnedMeshRenderer smr in allSkinnedMeshRenderers ){
		        ...
		        if( smr.sharedMesh != null && // Make sure there IS a shared Mesh in the first place!
		        	PrefabUtility.GetPrefabObject(smr.gameObject) != null && // This lets us know if this object is actually a prefab
		        	PrefabUtility.FindRootGameObjectWithSameParentPrefab(smr.gameObject) != null &&	// Can we grab the parent prefab
				...
				if( EditorUtility.DisplayDialog("Prefabs Are Using Local Meshes!", "MeshKit has detected that you are using Prefabs with meshes that are being managed for a specific Unity Scene. By making a change in that scene you may inadvertently delete the mesh in your prefab.\n\nMeshKit can automatically rebuild these Prefabs for you by duplicating their Mesh files into the Assets folder of your project where they will no longer be tracked by MeshKit (and you will be able to use them normally).\n\nWould you like MeshKit to automatically fix your Prefabs?", "Yes", "No")
					...
					if(meshfilters.Length > 0 ){
						foreach( MeshFilter mf2 in meshfilters ){
							if( mf2 != null && mf2.sharedMesh != null ){
								...
								if( PrefabUtility.FindRootGameObjectWithSameParentPrefab(mf2.gameObject) != null ){
									goFolderName = PrefabUtility.FindRootGameObjectWithSameParentPrefab(mf2.gameObject).name.MakeFileSystemSafe(' ');
								}
								...
								if(mf2!=null && mf2.sharedMesh != null ){
									...
									if( newMesh != null ){
										...
										if( PrefabUtility.FindValidUploadPrefabInstanceRoot( mf2.gameObject ) as GameObject != null ){
											...
											if( meshKitPrefabArray.Count > 0 ){
												foreach( MeshKitPrefabToSaveLater x in meshKitPrefabArray ){
													if(MeshKitGUI.verbose){ Debug.Log( "IF " + x.go + " == " + (PrefabUtility.FindValidUploadPrefabInstanceRoot(mf2.gameObject) as GameObject) ); }
													if( x!=null && x.go ==  PrefabUtility.FindValidUploadPrefabInstanceRoot(mf2.gameObject) as GameObject ){
											...
											if(exists == false){
												...
												savePrefabLater.go = PrefabUtility.FindValidUploadPrefabInstanceRoot(mf2.gameObject) as GameObject;
					...
					if(skinnedMeshRenderers.Length > 0 ){
						foreach( SkinnedMeshRenderer smr2 in skinnedMeshRenderers ){
							if( smr2 != null && smr2.sharedMesh != null ){
								...
								// If we can get a name from the root gameobject that would probably be better ...
								if( PrefabUtility.FindRootGameObjectWithSameParentPrefab(smr2.gameObject) != null ){
									goFolderName = PrefabUtility.FindRootGameObjectWithSameParentPrefab(smr2.gameObject).name.MakeFileSystemSafe(' ');
								}
								...
								if(smr2!=null && smr2.sharedMesh != null ){
									...
									if( newMesh != null ){
										...
										// Make sure we can access the prefab GameObject
										if( PrefabUtility.FindValidUploadPrefabInstanceRoot( smr2.gameObject ) as GameObject != null ){

											// Find the main GameObject of this prefab and check if its already in this new array
											bool exists = false;
											if( meshKitPrefabArray.Count > 0 ){
												foreach( MeshKitPrefabToSaveLater x in meshKitPrefabArray ){
													if(MeshKitGUI.verbose){ Debug.Log( "IF " + x.go + " == " + (PrefabUtility.FindValidUploadPrefabInstanceRoot(smr2.gameObject) as GameObject) ); }
													if( x!=null && x.go ==  PrefabUtility.FindValidUploadPrefabInstanceRoot(smr2.gameObject) as GameObject ){
														if(MeshKitGUI.verbose){ Debug.Log("Exists!"); }
														exists = true;
													}
												}
											...
											if(exists == false){
												...
												savePrefabLater.go = PrefabUtility.FindValidUploadPrefabInstanceRoot(smr2.gameObject) as GameObject;
					...
					if(meshKitPrefabArray.Count > 0 ){
						foreach( MeshKitPrefabToSaveLater p in meshKitPrefabArray ){
							...
							// Show Debug Info
							if( p != null && p.go != null &&
								// Make sure mesh filter is valid if it should be, or the the skinnedMeshRenderer is
							...
							if( p != null && p.go != null &&
								// Make sure mesh filter is valid if it should be, or the the skinnedMeshRenderer is
								( p.isMeshFilter == true && p.mf != null || p.isMeshFilter == false && p.smr != null  ) &&
								PrefabUtility.GetPrefabType(PrefabUtility.FindValidUploadPrefabInstanceRoot(p.go)) == PrefabType.PrefabInstance &&
								...
								PrefabUtility.ReplacePrefab( 	
																p.go,
																#if UNITY_2018_2_OR_NEWER
																	PrefabUtility.GetCorrespondingObjectFromSource(p.go), 
																#else
																	PrefabUtility.GetPrefabParent(p.go), 
																#endif
