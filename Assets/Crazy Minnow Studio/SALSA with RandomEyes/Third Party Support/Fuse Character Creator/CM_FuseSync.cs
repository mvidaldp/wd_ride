...
namespace CrazyMinnow.SALSA.Fuse
{
	...
	[AddComponentMenu("Crazy Minnow Studio/SALSA/Addons/Fuse Character Creator/CM_FuseSync")]
	public class CM_FuseSync : MonoBehaviour 
	{
        ...
        [Range(0f, 100f)]
        public float blinkEyelidPercent = 80f; // Eyelid blink percentage
        [Range(0f, 100f)]
        public float blinkEyelashPercent = 70f; // Eyelash blink percentage
		...
		private bool lockShapes; // Determines when to lock the shapes
		private bool unlockDelay; // Determines when to start the unlock delay
		private WaitForSeconds unlockDelayWait = new WaitForSeconds(0.1f); // Unlock delay duration
		...
		void LateUpdate() 
		{
			...
			if (salsa3D)
			{
				...
				if (!salsa3D.isTalking && unlockDelay && salsa3D.lockShapes != lockShapes)
				{
					unlockDelay = false;
					StartCoroutine(ShapeUnlockDelay());
				}
			}
            ...
            if (randomEyes3D)
            {
                ...
                for (int i = 0; i < body.sharedMesh.blendShapeCount; i++)
                {
                    ...
                    if (eyelashes)
                    {
                        ...
                        {
                            eyelashes.SetBlendShapeWeight(i, (srcShapeWeight * blinkEyelashPercent) / 100);
                        }
                    }
                    ...
                    if (facialHair.Count > 0)
                    {
                        for (int fh = 0; fh < facialHair.Count; fh++)
                        {
                            if (srcShapeWeight > 0)
                            {
                                ...
                                if (body.sharedMesh.GetBlendShapeName(i).Contains("."))
                ...
                if (body)
                {
                    ...
                    if (leftEyelidIndex != -1) body.SetBlendShapeWeight(leftEyelidIndex, (blinkWeight * blinkEyelidPercent) / 100);
                    if (rightEyelidIndex != -1) body.SetBlendShapeWeight(rightEyelidIndex, (blinkWeight * blinkEyelidPercent) / 100);
                }
		...
		/// <summary>
		/// Wait for all shapes to settle on zero before unlocking shapes
		/// </summary>
		/// <returns></returns>
		IEnumerator ShapeUnlockDelay()
		{
			yield return unlockDelayWait;
			lockShapes = false;
		}
